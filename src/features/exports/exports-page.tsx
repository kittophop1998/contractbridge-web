...
function ExportPreview({ file, title, description, icon }: { file: ExportFile; title: string; description: string; icon: "yaml" | "markdown" }) {
  ...
  return <Card><CardContent className="space-y-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-start gap-3"><span className="grid size-9 place-items-center rounded-md bg-accent/15 text-accent-foreground"><Icon className="size-4" aria-hidden /></span><div><h2 className="font-heading font-bold text-primary">{title}</h2><p className="text-sm text-muted-foreground">{description}</p></div></div><div className="flex shrink-0 gap-2"><CopyButton value={file.content} /><Button variant="outline" size="sm" onClick={() => download(file)}><DownloadIcon aria-hidden /> Download</Button></div></div><pre className="max-h-[55vh] overflow-auto rounded-md border bg-[#2c3e50] p-4 font-mono text-xs leading-5 text-[#ecf0f1]"><code>{file.content}</code></pre></CardContent></Card>;
}
